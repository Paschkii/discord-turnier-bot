...
module.exports = {
  async run(interaction) {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
      return interaction.reply({ content: '⛔ Nur Admins dürfen den Pott setzen.', ephemeral: true });
    }
    ...
    if (!totalMK || totalMK <= 0) {
      return interaction.reply({ content: '❌ Bitte gib einen gültigen Gesamtpott an (z. B. 15mk).', ephemeral: true });
    }
    if ((firstIn && firstMK === null) || (secondIn && secondMK === null) || (thirdIn && thirdMK === null)) {
      return interaction.reply({ content: '❌ Ungültiges Format bei den Platzprämien. Beispiele: 9mk, 4mk, 2000k, 15000000', ephemeral: true });
    }
