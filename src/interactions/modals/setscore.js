...
module.exports = {
  async run(interaction) {
    if (!interaction.member.permissions.has(PermissionsBitField.Flags.Administrator)) {
      return interaction.reply({ content: '⛔ Nur Admins dürfen Ergebnisse setzen.', ephemeral: true });
    }
    ...
    if (!fight) return interaction.reply({ content: `❌ Kampf #${kampfId} nicht gefunden.`, ephemeral: true });
    ...
    if (!Number.isInteger(a) || !Number.isInteger(b)) {
      return interaction.reply({ content: '❌ Bitte nur ganze Zahlen eingeben.', ephemeral: true });
    }
    ...
    return interaction.reply({ content: `🛠️ Ergebnis gesetzt: **#${kampfId}**${grpInfo} — ${fight.playerA.name} ${a} : ${b} ${fight.playerB.name}`, ephemeral: false });
  }
};
